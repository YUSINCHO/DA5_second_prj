...
퍼널 분석 시 유의했던 3가지
1. 유저 중복을 제거하고 유입과 이탈 모두 하나의 유저만을 대상
2. 이전 단계를 거친 세션만 포함하여 연속성 확인
3. 단계별 고유 세션 수 계산

퍼널 분석 결과 페이지 진입 후 step 1에서 47.4% 이탈율을 보이며 step4에서 마지막 단계 copmplete에서 20.8%의 이탈율을 보임

#### 인사이트
1. 지원서 작성에 지원자들이 step 1으로 넘어갈 때 심리적 장벽을 낮춰 줄 전략이 필요
2. 지원을 본격적으로 시작하면 지원을 완료할 가능성이 높음.

- 결론 : 지원자들이 지원서르 끝까지 작성할 수 있는 UI 및 전략들이 필요. 특히, step 4에서 제출 단계에 보이는 이탈율 20.8%를 주목하여 전략을 만들 필요성이 있음.
- 한계 : 각 단계별로 임시 저장 및 이전 단계 검토 등의 페이지 이탈은 반영하지 못함 -> 추후 추가 분석이 필요

### 회원가입 퍼널분석
![1팀_중급프로젝트 (2)](https://github.com/user-attachments/assets/01c93b65-8311-4f98-8000-e1d8d6bad315)
- 회원가입 페이지를 대상으로 퍼널 분석을 하게 된 이유는 모든 URL 경로에 step 이라는 path가 포함된 페이지는 apply와 회원가입만 존재
- 회원가입 퍼널의 경우 각 단계별로 다른 path들이 포함되어 있음.
- Step 별 상세한 경로는 하나의 Step으로 병합하여 퍼널 분석을 진행

![1팀_중급프로젝트 (3)](https://github.com/user-attachments/assets/f3e1e24a-c3e2-4b9b-bc8a-cd36240dfef1)
- 각 단계별로 이탈율이 평균적으로 30% 발생하는 것을 알 수 있음.
- 채용플랫폼의 특성상 회원가입의 목적성이 강하여 특정 스탭에서 이탈율이 두드러지지 않는 것으로 추측됨.


## 프로젝트에 대한 회고
1. 팀장으로서 맡은 바를 잘 수행했는가?
단도직입적으로 표현하자면 팀장으로의 역할을 제대로 수행하지 못했다 라고 생각된다.
팀원들의 결과물을 꼼꼼하게 살피고 피드백을 주어야 했다고 생각한다. 그렇다고 관망했다는 것이 아니라 미흡했다.
만약 다음에도 팀장 역할을 담당하게 된다면 나의 Task에 몰두 하는 것이 아닌 팀원들이 전체적으로 어떤 어려움이 있는지 어떤 챌린지를 하고 있는지 더욱 관심을 갖고 함께 해결하려고 노력해야 한다는 교훈을 얻었다.

1. 팀 분위기
이전 초급 프로젝트에 비해 정말 좋은 분위기었다. 모두가 웃을 땐 웃고 힘들 땐 같이 힘들어 했던 기억이 있다. 중급 프로젝트 팀들과 다음 프로젝트 진행하고 싶고 헤어지는게 아쉬울 정도로 분위기가 좋았다. 

1. 나의 챌린지
사실 이번 프로젝트를 통해 많은 성장을 했다고 느꼈다. 온전하지 못한 데이터셋을 통해 어떻게든 전처리하고 인사이트를 도출하려고 했던 도전들이 힘들기도 했지만 그 와중에 새로운 것을 발견했다는 것에 희비가 가득했던 것 같다.
